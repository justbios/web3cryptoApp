...
export default class TransactionsManagement implements ITransactionsManagement {
    ...
    async getTransaction(config: IGetTransactionsConfig): Promise<TransactionEntity[]> {
        const {status, data} = await this.axios.get<IEtherscanResult>(`/api?module=account&action=txlist&address=${config.address}&startblock=0&endblock=99999999&page=${config.offset}&offset=${config.limit}&sort=asc&apikey=${apiKey}`);
